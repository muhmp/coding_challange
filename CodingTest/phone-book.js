...
function PhoneBookForm(props) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (!userState.userFirstname || !userState.userLastname || !userState.userPhone) return;
    //addEntryToPhoneBook(userState); //userState
    props.addUser(userState);
    setUserState(initContact); //setUserState
};
...
function InformationTable(props) {
  //return list contact to the table
  const listContact = props.users.sort((x, y) => x.userLastname.localeCompare(y.userLastname));

  const display = listContact.length > 0 ? (
